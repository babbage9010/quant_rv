### quant_rv v1.3.3 by babbage9010 and friends
### released under MIT License
# changelog
# 1: add a second trade symbol for Short SPY (SH)
# 2: go short SPY (using SH) when not long SPY
...
signal_2 <- ifelse( !signal_1, 1, 0)
signal_2[is.na(signal_2)] <- 0
label_strategy2 <- "Strategy 2: Short MV5 (inverse Multvol 5)"

# don't use signal_3 here
#signal_3 <- ifelse(totalvol >= 1, 1, 0)
#signal_3[is.na(signal_3)] <- 0
label_strategy3 <- "Strategy 3: Multivol 5 Long/Short "
...
returns_strategy2 <- roc_trade2 * stats::lag(signal_2, 2)
returns_strategy2 <- na.omit(returns_strategy2)
# MV5 L/S combines Strategy_1 + _2
returns_strategy3 <- returns_strategy1 + returns_strategy2
...
for(s in 1:20){
  ...
  signal_2 <- ifelse(totalvol + totalnatr >= 1, 1, 0) #Multivol 5 with nATR
  signal_2[is.na(signal_2)] <- 0
  signal_2inv <- ifelse( !signal_2 , 1, 0) #Multivol 5 inverse 
  signal_2inv[is.na(signal_2inv)] <- 0

  # Step 4: Backtest the strategies
  returns_strategy2 <- roc_trade1 * stats::lag(signal_2, 2)
  returns_strategy2 <- na.omit(returns_strategy2)
  returns_strategy2inv <- roc_trade2 * stats::lag(signal_2inv, 2)
  returns_strategy2inv <- na.omit(returns_strategy2inv)
  returns_strategy2LS <- returns_strategy2 + returns_strategy2inv
  returns_strategy2LS <- na.omit(returns_strategy2LS)
  
  #rtns <- returns_strategy2 # MV5 
  rtns <- returns_strategy2LS  #MV5 L/S
  #rtns <- returns_strategy2inv  #MV5 inv (short SPY)
