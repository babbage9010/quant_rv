### rv_vs_rtns_0.0.3.R by babbage9010 and friends
# update code from v2 to produce plots for blog post: 
# babbage9010.wordpress.com/2023/10/25/part-10-quant_rv-getting-somewhere-now-by-adding-normalized-atr/
# added nATR as a vol measure
# Not intuitive, but it works, like this: 
# 1) to reproduce the B&W plots (1 lookback period)
#   a) set "switchy" (Line 119) to FALSE
#   b) choose vol signal as x_dat (L 48-53)
#   c) use numcols (L 74) to choose 2 or 3 column plot
#   
# 2) to reproduce the color, lookback spectrum plots   
#   a) set "switchy" (L 119) to TRUE
#   b) choose vol signal as x_dat (L 181-186)
#   c) play with maximum lookback if you want (L 164)
#   d) to exactly duplicate Y scales, see notes (L 127-138)
...
pricesAd <- na.omit( Ad(spy_data) )
pricesOp <- na.omit( Op(spy_data) )
pricesCl <- na.omit( Cl(spy_data) )
pricesHi <- na.omit( Hi(spy_data) )
pricesLo <- na.omit( Lo(spy_data) )
...
rs   <- volatility(spy_data, n=lookbk, calc="rogers.satchell")
gkyz <- volatility(spy_data, n=lookbk, calc="gk.yz")
park <- volatility(spy_data, n=lookbk, calc="parkinson")
avgvol <- (rv+rs+gkyz+park) / 4
nATR <- ATR(spy_data, n=lookbk, maType="ZLEMA")[ , "atr"] / pricesCl
...
numrows <- 3 
numcols <- 3 #either 2 or 3 columns please
...
if(switchy){
  ...
  for(lb in lbaks){
    ...
    rs   <- volatility(spy_data, n=lb, calc="rogers.satchell")
    gkyz <- volatility(spy_data, n=lb, calc="gk.yz")
    park <- volatility(spy_data, n=lb, calc="parkinson")
    avg4 <- (rv+rs+gkyz+park) / 4
    nATR <- ATR(spy_data, n=lb, maType="ZLEMA")[ , "atr"] / pricesCl
    ...
    print(paste(lb,i,netgain2[1], annvols[1], rvratios[1]))
  }
  figs1 <- list()
  figs2 <- list()
  figs3 <- list()
